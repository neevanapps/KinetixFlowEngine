...
namespace KinetixFlowEngine.Core.Engine
{
    public class KinetixEngineProcessor
    {
        ...
        public KinetixEngineProcessor(FlowFeatureEngine flowFeatureEngine, 
             VwapEngine vwapEngine, EfficiencyRatioEngine erEngine, AtrEngine atrEngine, OpenInterestEngine oiEngine, 
            PriceTrendEngine priceEngine, SignalStabilityEngine signalStabilityEngine,
            ...
            FlowPersistenceEngine flowPersistenceEngine, FlowTradeBuffer tradeBuffer, FifteenMinuteCandleBuilder candle15m, VolumeEngine volumeEngine, FlowMomentumRun momentumRun,
        ...
        public KinetixEngineResult Process(double price, decimal quantity, long timeStamp, double openInterest, double fundingRate, double fundingPressure)
        {
            return new KinetixEngineResult
            {
                ...
                Volume15 = 0,
                Volume1 = 0,
                VWAP = (double)_vwapEngine.Update((decimal)price, quantity),
                ER = _erEngine.Update(price),
                ER5 = _erEngine.Update(price),
                ER30 = _er30m.Update(price),
                ATR = _atrEngine.Update(price, price, price),
                OIChange = _oiEngine.Update(openInterest),
            ...
            //_atrNormalizer.Update(atr);
            //double atrNorm = _atrNormalizer.GetNormalized(atr);
            //double atrScale = _atrNormalizer.GetScale(atrNorm);
            //double finalScore = adjustedScore / atrScale;
